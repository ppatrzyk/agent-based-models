globals [generation payoffs strategies fit_noone fit_altruistic fit_spiteful fit_antisocial all_obs b1 b2 pun1 pun2 DDs CDs CCs fights dominations deterred]

breed [agents agent]

agents-own [partnered observe decision fitness strength feelstrong strategy fear]

to setup
  clear-all
  set payoffs [[6 0][10 2]] ;cooperate: 0, defect: 1
  set strategies ["noone" "altruistic" "spiteful" "antisocial"]
  set generation 1
  create num_noone num_altruistic num_spiteful num_antisocial
  agent-look
  reset-ticks
end

; helper for creating agents with different strategies
to create [noone altruistic spiteful antisocial]
  create-agents noone [
...
to pair
  ...
  let pairs floor (total / 10) ;1/5 agents chosen to be PD players
  repeat pairs [
...
to compete [sens]
  ...
  ; set deterrence if opponent is in fear list
  ...
  [set b2 deterrence]
  [set b2 0]
  let util1 sigmoid (s1 - (s2 + b1)) sens ;strength of the opponent is overestimated if he's in fear list
  ...
  ask end1 [
    ifelse feelstrong = 0 ;cooperate if you think you're weaker, otherwise defect
    [set decision 0]
    [set decision 1]     
  ]
  ask end2 [
...
to punish [sens]
  ...
  let p2 end2
  choose-observers
  ; conflict escalation - determine willingness to punish
  if dec1 = 1 and dec2 = 1 ;DD case
  [
    if (str1 = "altruistic" or str1 = "spiteful")
    [set pun1 1]
    if (str2 = "altruistic" or str2 = "spiteful")
    [set pun2 1]
    ]
  if dec1 = 1 and dec2 = 0 ;DC case
  [
    if (str1 = "antisocial" or str1 = "spiteful")
    [set pun1 1]
    ]
  if dec1 = 0 and dec2 = 1 ;CD case
  [
    if (str2 = "antisocial" or str2 = "spiteful")
    [set pun2 1]
    ]
  ...
  [
    ...
    [ ;agent end1 wins
      ...
      foreach all_obs [
            ...
            ask n-of gossip agents [set fear fear_update p1 fear]
            ]
      ]
    [ ;agent end2 wins
      ask end1 [
      ...
      foreach all_obs [
            ...
            ask n-of gossip agents [set fear fear_update p2 fear]
            ]
      ]
    ]
  if pun1 = 1 and pun2 = 0
  [
    set dominations (dominations + 1)
    ask end2 [set fear fear_update p1 fear]
    foreach all_obs [
            ask ? [set fear fear_update p1 fear]
            ask n-of gossip agents [set fear fear_update p1 fear]
            ] 
  ...
  [
    set dominations (dominations + 1)
    ask end1 [set fear fear_update p2 fear]
    foreach all_obs [
            ask ? [set fear fear_update p2 fear]
            ask n-of gossip agents [set fear fear_update p2 fear]
            ] 
...
to replicate
  ...
  let all_strategies (weight_noone + weight_altruistic + weight_spiteful + weight_antisocial)
  ...
  ask agents [die]
  let diff total - ((round(new_noone * total)) + (round(new_altruistic * total)) + (round(new_spiteful * total)) + (round(new_antisocial * total)))
  create (round(new_noone * total)) (round(new_altruistic * total)) (round(new_spiteful * total)) (round(new_antisocial * total))
  ifelse diff > 0
  [repeat diff [create 1 0 0 0]]
...
"" ""
PENS
"fight" 1.0 0 -2674135 true "" "plotxy generation fights / (fights + dominations + deterred + 0.1)"
"dominate" 1.0 0 -13345367 true "" "plotxy generation dominations / (fights + dominations + deterred + 0.1)"
"deterred" 1.0 0 -10899396 true "" "plotxy generation deterred / (fights + dominations + deterred + 0.1)"
...
@#$#@#$#@
<experiments>
  <experiment name="M1run" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>CDs</metric>
    <metric>CCs</metric>
    <metric>fights</metric>
    <metric>dominations</metric>
  ...
  <experiment name="test" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="num_antisocial">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="num_spiteful">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="submit_cost">
      <value value="6"/>
    ...
    <enumeratedValueSet variable="num_altruistic">
      <value value="0"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="punishment">
      <value value="9"/>
